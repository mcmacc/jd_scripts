      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data.data && data.data.iconArea) {	            $.activityId = data.activityId
              let act = data.data.iconArea.filter(vo=>vo['type']==="platform_red_packege_rain")[0]	            $.st = data.startTime
              if (act) {	            $.ed = data.endTime
                let url = act.data.activityUrl	            console.log(`下一场红包雨id：${$.activityId}`)
                $.activityId = url.substr(url.indexOf("id=") + 3)	            console.log(`下一场红包雨开始时间：${new Date(data.startTime)}`)
                $.st = act.startTime	            console.log(`下一场红包雨结束时间：${new Date(data.endTime)}`)
                $.ed = act.endTime	
